...
/**
 * This application asks user to use whole dollar mode or currency mode. It holds them in a
 * while loop until a valid option is selected. It then directs to the appropriate method.
 * 
 * @author Jon Cesena
 * @version v1.0
 * @since 3/30/2026
...
public class BankAccountDemo{  //Update to Main for Replit or MyProgram for codeHS
    public static void main (String[] args){
        ...
        while (!(number==1||number==2)){    //Forces a 1 or 2 input
            System.out.print("Press 1 to use whole dollars or 2 to use currency: ");
            number = keyboard.nextInt();
        }
        if (number==1){ //calls useWholeDollarMode()
            useWholeDollarMode();
        }
        else{   //calls useCurrencyMode()
            useCurrencyMode();
        }
    }//end main
